...
class Forward_pass_numpy():
    ...
    def acc_prec(self, y_true, y_pred):
        "Calculates the accruacy and precision of the model"
        self.y_true = y_true
        self.y_pred = y_pred
        N = len(self.y_true)

        acc = (self.y_true == self.y_pred).sum() / N #accuracy
        TP = ((self.y_pred == 1) & (self.y_true == 1)).sum() #True Positive
        FP = ((self.y_pred == 1) & (self.y_true == 0)).sum() #False Positive

        precision = TP/(TP+FP)

        return acc * 100, precision * 100
    ...
    def nn_forward_pass(self, A, seed):
        ...
        #Printing loss
        axes[1, 0].bar([1,2], [self.bce, 0], width=0.7)
        plt.xlabel('Loss')
        plt.ylabel('Loss')
        ...
        self.acc, self.prec = self.acc_prec(self.y_true, self.y_pred)
        print(f"Accuracy of model: {self.acc}%, Precision of model: {self.prec}%")
        fig, axes = plt.subplots(1, 1, figsize = (12, 10))
        axes.bar([1, 2], [self.acc, self.prec], width=0.7, tick_label = ['Acc', 'Prec'], align='center')
        plt.xlabel('Accuracy and Precision')
